   ...
   lastGot(-1)
{
    ...
    if(type == CameraType::OpenNI2)
      cam = new OpenNI2Interface(width,height);
    else
      cam = nullptr;
...
void LiveLogReader::getNext()
{
    ...
    memcpy(&decompressionBufferDepth[0], cam->frameBuffers[bufferIndex].first.first, numPixels * 2);
    memcpy(&decompressionBufferImage[0], cam->frameBuffers[bufferIndex].first.second,numPixels * 3);
